...
Live system health dashboard — FastAPI + Jinja2 + HTMX with brutalist CMYK design. 7 pages: health overview, registry browser, dependency graph, soak test monitoring, essay feed, omega scorecard, IRF dashboard.
...
| `/irf/` | Index Rerum Faciendarum dashboard — stats summary, P0 items, open items by priority/domain, completed items |
...
### Siblings in Meta
`.github`, `organvm-corpvs-testamentvm`, `alchemia-ingestvm`, `schema-definitions`, `organvm-engine`, `organvm-mcp-server`, `praxis-perpetua`, `stakeholder-portal`, `materia-collider`, `organvm-ontologia`, `vigiles-aeternae--agon-cosmogonicum`, `cvrsvs-honorvm`
...
Run: `organvm ecosystem show system-dashboard` | `organvm ecosystem validate --organ META`


## External Mirrors (Network Testament)

- **technical** (3): tiangolo/fastapi, encode/uvicorn, pallets/jinja

Convergences: 20 | Run: `organvm network map --repo system-dashboard` | `organvm network suggest`


## Task Queue (from pipeline)

**15** pending tasks | Last pipeline: unknown

- `cd60d3d20543` src/dashboard/formatters/__init__.py — Create [bash, cloudflare, fastapi]
- `f808316d7b43` src/dashboard/formatters/plaintext.py — Create [bash, cloudflare, fastapi]
- `7a26eb23fed7` src/dashboard/routes/api_v1.py — Modify [bash, cloudflare, fastapi]
- `29a63d8ad865` tests/test_api_v1.py — Modify [bash, cloudflare, fastapi]
- `03e853939d94` infra/termius-setup.md — Create [bash, cloudflare, fastapi]
- `997c0bc9ff26` [x] Explore codebase structure [cloudflare, fastapi, mcp]
- `dc4c2b235215` [ ] Add dependencies to pyproject.toml [cloudflare, fastapi, mcp]
- `dbc4d858ad39` [ ] Create auth middleware (`src/dashboard/middleware/auth.py`) [cloudflare, fastapi, mcp]
- ... and 7 more
...
**UID:** `ent_repo_01KKKX3RVR10FGQAX0EQ31J561` | **Matched by:** primary_name

Resolve: `organvm ontologia resolve system-dashboard` | History: `organvm ontologia history ent_repo_01KKKX3RVR10FGQAX0EQ31J561`
